...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const rolUser = form.querySelector('input[name="rol-user"]');
    ...
    function validatePassword() {
        ...
        if (value && (value.length < 8 || !hasUpperCase || !hasNumber)) {
    ...
    function validateRolUser() {
        const value = rolUser.value;
        if (!value) {
            showError(rolUser, 'El ID Rol no puede estar vacío.');
        } else if (isNaN(value)) {
            showError(rolUser, 'El ID Rol debe ser un número.');
        } else {
            showSuccess(rolUser);
        }
    }
    ...
    rolUser.addEventListener('input', validateRolUser);
